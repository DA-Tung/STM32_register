...
//
void usart_transmit_multi_data(USART_TypeDef* USARTx, uint8_t *str, uint16_t size)
{
	while( size > 0)
	{
		// Waiting TXE set high after the stop bit		
		while(!(USARTx->SR & USART_SR_TXE));
		
		// Transmit data		
		USARTx->DR = *str;	
		
		// increase to next data
		str++;
		
		// decrease size after transmit data
		size--;
	}
	// After writing the last data anh wait unitl TC = 1	
	while(!(USARTx->SR & USART_SR_TC));
