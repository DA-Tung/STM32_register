...
#define CMD_GRID				0x02				// Generates a 10-byte random ID number
#define CMD_CALCCRC           	0x03               	// CRC Calculate
#define CMD_TRANSMIT          	0x04               	// Transmit data
#define CMD_NO_CMD_CHANGCE		0x07				// no command change, can be used to modify the CommandReg register bits without affecting the command, for example, the PowerDown bit
...
void RC522_Write_Data(uint8_t add_reg, uint8_t data_write);
uint8_t RC522_Read_Data(uint8_t add_reg);
void SetBitMask(uint8_t reg, uint8_t mask);
void ClearBitMask(uint8_t reg, uint8_t mask);
void RC52_SoftReset(void);
