...
//SPI Chip Select
#define _LIS3DHS_CS_ENBALE		gpio_output(GPIOE,PIN3,PIN_RESET);
#define _LIS3DHS_CS_DISABLE		gpio_output(GPIOE,PIN3,PIN_SET);
//Library variables
//1. SPI handle
#define SPI_LIS3DSH 			SPI1
...
void LIS3DSH_WriteIO(uint8_t reg, uint8_t *dataW, uint8_t size)
{
	...
	_LIS3DHS_CS_ENBALE;
	//set register value
	spi_transmit(SPI_LIS3DSH, &spiReg, 1);
	//Transmit data
	spi_transmit(SPI_LIS3DSH, dataW, size);
	//Disable CS
	_LIS3DHS_CS_DISABLE;
}
//2. Read IO
void LIS3DSH_ReadIO(uint8_t reg, uint8_t *dataR, uint8_t size)
{
	...
	_LIS3DHS_CS_ENBALE;
	//set register value
	spi_transmit(SPI_LIS3DSH, &spiBuf[0], 1);
	//Transmit data
	spi_receive(SPI_LIS3DSH, spiBuf, size);
	//Disable CS
	_LIS3DHS_CS_DISABLE;
...
void LIS3DSH_Init(LIS3DSH_InitTypeDef *accInitDef)
{
	uint8_t spiData = 0;
